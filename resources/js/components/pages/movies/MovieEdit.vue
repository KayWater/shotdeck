        ...
        <div class="container mt-10">
            ...
            <el-form 
                ...
                <el-row >
                    ...
                    <el-form-item>
                        <el-button @click="handleUpdateMovie">保存</el-button>
                    </el-form-item>
                </el-row>
            </el-form>
        </div>
...
export default {
    ...
    methods: {
        ...
        handleUpdateMovie() {
            ...
            this.$refs['form'].validate((valid) => {
                ...
                this.updateMovie(this.form).then((response) => {
                    vm.$message({
                        message: '更新成功',
                        type: 'success',
                    });
    ...
    created() {
        ...
        this.getMovie(this.id).then((response) => {
            ...
            this.form.name = movie.name;
            this.form.nameEn = movie.name_en;
            this.form.intro = movie.intro;
            this.form.posterUrl = 'storage/' + movie.poster_url;
            this.form.releaseTime = movie.release_time;
            this.form.duration = movie.duration;
            this.form.shotCount = movie.shot_count;
            this.form.genres = movie.genres.map((item) => { return item.id });
            this.form.regions = movie.regions.map((item) => { return item.id });
            let directors = Array();
            let actors = Array();
            movie.people.forEach((item) => {
                if (item.pivot.role_type == 0) {
                    this.directorOptions.push(item);
                    directors.push( item.id );
                } else if (item.pivot.role_type == 1) {
                    let actor = {
                        id: [item.id],
                        type: item.pivot.role_type+'',
                        name: item.pivot.character,
                        description: item.pivot.character_description,
                    }
                    this.actorOptions.push(item);
                    actors.push( actor );
                }
            });
            this.form.directors = directors;
